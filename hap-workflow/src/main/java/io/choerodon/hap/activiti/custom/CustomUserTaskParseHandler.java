...
import io.choerodon.hap.activiti.listeners.AutoDelegateListener;
import io.choerodon.hap.activiti.listeners.NotificationListener;
import io.choerodon.hap.activiti.listeners.TaskCreateNotificationListener;
import io.choerodon.hap.activiti.listeners.TaskEndNotificationListener;
...
public class CustomUserTaskParseHandler extends UserTaskParseHandler implements IActivitiConstants {
    ...
    private void autoEnd(BpmnParse bpmnParse, FlowNode userTask) {
        ...
        if (nextNode instanceof Task) {
            ...
            s1.setTargetFlowElement(exclusiveGateway);
            s1.setWaypoints(s0.getWaypoints());
            exclusiveGateway.setIncomingFlows(Collections.singletonList(s1));
