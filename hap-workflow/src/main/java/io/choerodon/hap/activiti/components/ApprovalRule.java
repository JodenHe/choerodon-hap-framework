package io.choerodon.hap.activiti.components;
    ...
    public Set<String> processCandidate(UserTask task, DelegateExecution execution) {
        ...
        for (FormValue value : values) {
            ...
            if (candidateRule == null) {
                logger.warn(String.format("code :%s is disable or not exist", strategyCode));
                continue;
            }
            jsonVaule = value.getName();
            try {
                ruleJson = objectMapper.readTree(jsonVaule);
            } catch (IOException e) {
                logger.warn(e.getMessage());
            }
            if (ruleJson == null || !hasPermission(ruleJson, execution)) {
                continue;
            }
                ...
                try {
                    ...
                    if (user != null) {
                        ...
                        if (user instanceof String) {
                            ...
                            logger.error(String.format("expression %s result type must be String, List or Set .not support %s", candidateRule.getExpression(), user.getClass().toString()));
                        }
                        if (users != null) {
            ...
            if (APPOINTED_POSITION.equalsIgnoreCase(ruleCode)) {
                ...
                if (StringUtils.isNotEmpty(positionCode)) {
                    ...
                    if (position != null) {
                        ...
                        logger.error(String.format("no position with code:%s", positionCode));
                    }
                }
                ...
                if (StringUtils.isNotEmpty(employeeCode)) {
                    ...
                    if (employee != null) {
                        ...
                        logger.error(String.format("no employee with code:%s", employeeCode));
                    }
                }
                ...
                if (StringUtils.isNotEmpty(roleCode)) {
                    ...
                    RequestHelper.clearCurrentRequest();
                    if (role != null) {
                        index.put(ASSIGNEE, role.getCode());
                        index.put(ASSIGNEE_NAME, role.getName());
                    } else {
                        logger.error(String.format("no employee with code:%s", roleCode));
                    }
                }
            }
            if (record != null) {
        ...
        if (businessRules != null) {
            ...
            for (JsonNode index : businessRules) {
                ...
                if (rule != null) {
                    ...
                    logger.error(String.format("no business rule  with code%s", ruleCode));
                }
