...
import { Table, Modal, Button, Dropdown, Menu, Icon } from 'choerodon-ui/pro';
...
export default class JobDetail extends PureComponent {
  ...
  schedulerMenu = (
    <Menu onClick={this.handleMenuClick}>
      <Item>
        <a onClick={() => this.handleStartSeched()} className={CLASS_NAME_MAP.green}><Icon type="play_arrow" />{$l('hap.resume')}</a>
      </Item>
      <Item>
        <a onClick={() => this.handleStandbySeched()} className={CLASS_NAME_MAP.yellow}><Icon type="pause" />{$l('hap.pause')}</a>
      </Item>
    </Menu>
  );
  ...
  newJobDropDown = (
    <Dropdown overlay={this.addJobMenu}>
      <Button funcType="flat" color="blue" icon="add">{$l('job.newjob')}<Icon type="arrow_drop_down" onClick={this.handleToggleDropdown} /></Button>
    </Dropdown>
  );
  ...
  jobMenu = (
    <Menu onClick={this.handleMenuClick}>
      <Item>
        <a onClick={() => this.handleResumeJob()} className={CLASS_NAME_MAP.green}><Icon type="play_arrow" />{$l('hap.resume')}</a>
      </Item>
      <Item>
        <a onClick={() => this.handlePauseJob()} className={CLASS_NAME_MAP.yellow}><Icon type="pause" />{$l('hap.pause')}</a>
      </Item>
      <Item>
        <a onClick={() => this.handleDeleteJob()} className={CLASS_NAME_MAP.red}><Icon type="delete" />{$l('hap.delete')}</a>
      </Item>
    </Menu>
  );
  ...
  render() {
    ...
    return (
      <Table buttons={[schedulerDropDown, this.newJobDropDown, this.jobDropDown]} dataSet={this.props.jobInfo} queryFieldsLimit={2}>
        <Column
          width={225}
